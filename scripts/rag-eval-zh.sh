...
Options:
  ...
  --output-file PATH     Markdown report path. Default: data/eval_zh_answers.md
EOF
      exit 0
      ;;
    *)
...
printf 'number\tquestion\texpected_group\tretrieval_strength\tevidence_diversity\ttop1\ttop5\tcoverage\tevidence_count\tresult_file\n' > "$results_tsv"
...
while IFS= read -r line; do
  ...
  retrieval_strength="$(sed -n 's/^Retrieval strength: //p' "$result_file" | head -n 1)"
  if [[ -z "$retrieval_strength" ]]; then
    retrieval_strength="$(sed -n 's/^Retrieval confidence: //p' "$result_file" | head -n 1)"
  fi
  evidence_diversity="$(sed -n 's/^Evidence diversity: //p' "$result_file" | head -n 1)"
  ...
  retrieval_strength="${retrieval_strength:-UNKNOWN}"
  evidence_diversity="${evidence_diversity:-UNKNOWN}"
  top1="${top1:-0}"
  top5="${top5:-0}"
  coverage="${coverage:-0}"
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$number" "$question" "$current_group" "$retrieval_strength" "$evidence_diversity" "$top1" "$top5" "$coverage" "$evidence_count" "$result_file" >> "$results_tsv"
done < "$questions_file"
...
retrieval_strength_counts = {"HIGH": 0, "MEDIUM": 0, "LOW": 0, "UNKNOWN": 0}
evidence_diversity_counts = {"HIGH": 0, "MEDIUM": 0, "LOW": 0, "UNKNOWN": 0}
for row in rows:
    retrieval_strength = row["retrieval_strength"]
    evidence_diversity = row["evidence_diversity"]
    retrieval_strength_counts[retrieval_strength] = retrieval_strength_counts.get(retrieval_strength, 0) + 1
    evidence_diversity_counts[evidence_diversity] = evidence_diversity_counts.get(evidence_diversity, 0) + 1
...
with output_path.open("w", encoding="utf-8") as out:
    ...
    out.write(f"- Retrieval strength HIGH count: {retrieval_strength_counts.get('HIGH', 0)}\n")
    out.write(f"- Retrieval strength MEDIUM count: {retrieval_strength_counts.get('MEDIUM', 0)}\n")
    out.write(f"- Retrieval strength LOW count: {retrieval_strength_counts.get('LOW', 0)}\n")
    out.write(f"- Evidence diversity HIGH count: {evidence_diversity_counts.get('HIGH', 0)}\n")
    out.write(f"- Evidence diversity MEDIUM count: {evidence_diversity_counts.get('MEDIUM', 0)}\n")
    out.write(f"- Evidence diversity LOW count: {evidence_diversity_counts.get('LOW', 0)}\n")
    unknown = sum(v for k, v in retrieval_strength_counts.items() if k not in {"HIGH", "MEDIUM", "LOW"})
    if unknown:
        out.write(f"- Retrieval strength UNKNOWN/other count: {unknown}\n")
